...
export async function POST(request) {
  ...
  const ipGuard = await checkRateLimit({ scope: "manage-cancel-ip", id: ip, limit: 8, windowMs: 60_000 });
  if (!ipGuard.ok) return NextResponse.json({ error: "Rate limit IP" }, { status: 429 });
  const phoneGuard = await checkRateLimit({ scope: "manage-cancel-token", id: parsed.data.token, limit: 5, windowMs: 60_000 });
  ...
  const byPhone = await checkRateLimit({ scope: "manage-cancel-phone", id: phoneId, limit: 5, windowMs: 60_000 });
